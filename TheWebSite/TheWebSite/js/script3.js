...
//Function to automate player 2 (UNFIXED BUG: 1 token deploy not working)

function checkTokensInRange() {
    // Only check if web is idle and player has tokens 
    if (player2.web.state === "idle" && player2Tokens > 0) {
        if (random() < 0.8) {
            // Check each power token
            for (let token of powerTokens) {
                // Calculate distance from player2 to token
                const distanceToToken = dist(player2.body.x, player2.body.y, token.x, token.y);
                // Calculate the maximum shoot range based on tokens in bank (50 pixels per token)
                const maxRange = player2Tokens * 50;
                // If token is within range
                if (distanceToToken <= maxRange) {
                    // Calculate angle needed to aim at token
                    //Oop don't ask...idk how the tan works (Courtesy of Claude)
                    const angleToToken = atan2(token.x - player2.body.x, token.y - player2.body.y);
                    // Set player2's rotation to aim at token
                    player2.body.rotation = -angleToToken;
                    // Deploy web
                    player2.web.state = "outbound";
                    player2.web.distance = 0;
                    break; // Exiting for loop after deploying web
                }
            }
        }
    }
